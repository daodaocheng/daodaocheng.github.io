    ...
    <div class="ht-content">
      <div class="ht-login">
        ...
        <div class="ht-login-content">
          <div id="app_login">
            ...
            <div class="ht_login_content_bottom">
              <div class="checkbox">
                <input
                  v-model="formLogin.checked"
                  id="remember"
                  type="checkbox"
                />
                <label for="remember" style="color: #ddd">记住账号</label>
              </div>
              <button @click="forgetPswd" class="ht_login_forget_btn">
                忘记密码？
              </button>
            </div>
          </div>
        ...
        <div class="ht-login-footer">
          <button @click="loginNavigate" class="ht-login-footer-btn">
            安全登录
          </button>
        </div>
  ...
  setup() {
    ...
    let loginNavigate = () => {
      if (formLogin._value.messageUserName == "") {
        ...
        if (formLogin._value.messagePassword == "") {
          ...
          // };
          if (
            formLogin._value.messageUserName == "admin" &&
            formLogin._value.messagePassword == "admin"
          ) {
            //console.log("name", formLogin._value.messageUserName);
            proxy.$router.push({
              path: "/index",
              query: {
                name: formLogin._value.messageUserName,
              },
            });
          } else {
            if (
              ...
              //alert("密码错误！");
              errorLog.value = "密码错误！";
            } else if (formLogin._value.messageUserName != "admin") {
              //alert("该账号不存在！");
              errorLog.value = "该账号不存在！";
            } else {
              //alert("登录失败！");
              errorLog.value = "登录失败！"
            }
    ...
    let setUserInfo = () => {
      if (formLogin._value.checked) {
        let loginId = JSON.stringify(formLogin._value.messageUserName);
        localStorage.setItem("idStr", loginId);
      } else {
        localStorage.setItem("idStr", "");
      }
    };
    if (localStorage.idStr && localStorage.idStr != undefined) {
      let username = JSON.parse(localStorage.idStr);
